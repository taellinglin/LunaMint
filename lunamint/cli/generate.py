...
Can be used as standalone: python -m lunamint.cli.generate --name NAME --user_id ID
Or imported: from lunamint.cli.generate import generate_for_user
"""
...
try:
    from lunamint.graphics.generate_banknote_front import (
        generate_single_banknote as generate_front,
    )
...
try:
    from lunamint.graphics.generate_banknote_back import (
        run_single_denomination as generate_back,
    )
...
# SD API is configured via lunamint.widgets.config (env-based).
...
def create_digital_banknote_signature(name, denomination, serial_number, timestamp_ms):
    ...
    try:
        ...
        result = generate_banknote_pair_svgs_pngs(
            name=name,
            denom=int(denom),
            portrait_path=img_path,
            output_dir=denom_folder,
        ...
        front_serial = result["front_serial"]
        back_serial = result["back_serial"]
        ...
        front_svg_path = result["front_svg"]
        back_svg_path = result["back_svg"]
        front_png_path = result["front_png"]
        back_png_path = result["back_png"]
...
def generate_front_back_pair(name, denom, img_path, timestamp_ms, denom_folder, user_id=None,
    ...
    def generate_back_task():
        ...
        try:
            ...
            print(f"[DEBUG] (FORCED) Calling run_single_denomination with outdir={denom_folder}, base_name={back_basename}, denomination={denom_str}, width_mm={width_mm}, height_mm={height_mm}, title_text={title}, phrase_text={subtitle}, seed_text={name}, serial_id={back_serial}, timestamp={int(timestamp_ms)}, png=False, dpi={dpi}, bg_image={bg_image}")
            from lunamint.graphics.generate_banknote_back import run_single_denomination
            run_single_denomination(
                outdir=denom_folder,
