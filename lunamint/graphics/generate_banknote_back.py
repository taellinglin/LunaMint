...
try:
    from ..widgets.config import sdapi_txt2img
    from ..widgets.crypto import build_qr_url, sm2_sign_bytes
    from ..graphics.modules.qr import (
        AZTEC_AVAILABLE,
        add_colored_aztec_to_canvas as _module_add_colored_aztec_to_canvas,
    ...
    from lunamint.widgets.config import sdapi_txt2img
    from lunamint.widgets.crypto import build_qr_url, sm2_sign_bytes
...
def _sm2_sign_bytes(data: bytes, private_key: str, public_key: str | None = None) -> bytes:
    return sm2_sign_bytes(data, private_key, public_key=public_key)
...
def add_security_background(
    ...
    if not sm2_public_key:
        sm2_public_key = os.getenv("LUNA_SM2_PUBLIC_KEY")

    payload = f"{seed_text or ''}|{denomination}|{timestamp_ms or ''}|{serial_id or ''}".encode("utf-8")

    if sm2_private_key:
        seed_hash = _sm2_sign_bytes(payload, sm2_private_key, public_key=sm2_public_key)
    elif serial_id:
        seed_hash = hashlib.sha3_512(serial_id.encode("utf-8")).digest()
    elif seed:
        seed_hash = seed
    else:
        seed_hash = hashlib.sha3_512(payload).digest()
    ...
    cell = max(6, int(base_triangle_size * 3))
    for y in range(margin, H - margin, cell):
        for x in range(margin, W - margin, cell):
            sat_byte = seed_hash[seed_i % seed_len]
            saturation = byte_to_saturation(sat_byte)
            seed_i += 1
            hue = (saturation * 360 + denomination) % 360
            r, g, b = colorsys.hsv_to_rgb(hue / 360, 0.25, 1.0)
            hex_color = rgb_to_hex((int(r * 255), int(g * 255), int(b * 255)))

            op_byte = seed_hash[seed_i % seed_len]
            opacity = 0.06 + (op_byte / 255.0) * 0.18
            seed_i += 1

            diag = seed_hash[seed_i % seed_len] % 2
            seed_i += 1

            if diag == 0:
                dwg.add(dwg.line(start=(x, y), end=(x + cell, y + cell), stroke=hex_color, stroke_width=1.0, opacity=opacity))
            else:
                dwg.add(dwg.line(start=(x, y + cell), end=(x + cell, y), stroke=hex_color, stroke_width=1.0, opacity=opacity))

            if (seed_hash[seed_i % seed_len] % 5) == 0:
                seed_i += 1
                dwg.add(dwg.rect(insert=(x + cell * 0.2, y + cell * 0.2), size=(cell * 0.6, cell * 0.6), fill=hex_color, fill_opacity=opacity * 0.6, stroke="none"))
...
def generate_sd_background(seed_text: str, width: int, height: int, 
    ...
    """
    save_path = str(save_path).strip().strip('"').strip("'")
    if not save_path:
        save_path = "./backgrounds"
    os.makedirs(save_path, exist_ok=True)
...
def generate_backside_svg(
    ...
    if progress_callback:
        progress_callback("start:back")
    print(f"[DEBUG] Starting back SVG generation: {outfile}")
    print(f"[DEBUG] generate_backside_svg: Called with outfile={outfile}, denomination={denomination}, title_text={title_text}, phrase_text={phrase_text}, size_px={size_px}, serial_id={serial_id}, timestamp={timestamp_ms}, seed_text={seed_text}")
    W, H = size_px
    ...
    if progress_callback:
        progress_callback("step:svgwrite_init")
    try:
        dwg = svgwrite.Drawing(outfile, size=(W,H), viewBox=f"0 0 {W} {H}")
    except Exception as e:
        print(f"[ERROR] Failed to create svgwrite.Drawing: {e}")
        import traceback
        traceback.print_exc()
        if progress_callback:
            progress_callback("error:svgwrite_init")
        raise
        if progress_callback:
            progress_callback("step:embed_fonts")
        print(f"[DEBUG] Embedding fonts and drawing background rectangle")
    ...
    if progress_callback:
        progress_callback("step:vectorizing_background")
    print(f"[DEBUG] Vectorizing background")
    ...
    if progress_callback:
        progress_callback("step:borders_and_seals")
    print(f"[DEBUG] Adding borders and seals")
    ...
    qr_url = build_qr_url(serial_id, denom_value, int(timestamp_ms))
    if progress_callback:
        progress_callback("step:qr_and_aztec")
    print(f"[DEBUG] Adding QR and aztec elements")
    ...
    if progress_callback:
        progress_callback("step:saving_svg")
    print(f"[DEBUG] Saving back SVG: {outfile}")
    try:
        dwg.save()
        print(f"[+] Saved {outfile}")
        if progress_callback:
            progress_callback("completed:back")
        print(f"[DEBUG] Back SVG saved: {outfile}")
    except Exception as e:
        print(f"[ERROR] Failed to save SVG to {outfile}: {e}")
        if progress_callback:
            progress_callback("error:saving_svg")
        import traceback
        traceback.print_exc()
        raise
import qrcode
...
def add_colored_aztec_to_canvas(
    ...
    return _module_add_colored_aztec_to_canvas(
        dwg,
        cx,
        cy,
        matrix,
...
def add_colored_aztecs(
    ...
    add_colored_aztec_to_canvas(
        ...
        matrix=matrix,
        denom_exponent=0,
        scale=max(2, size // max(1, len(matrix))),
        rotation=0,
        border_opacity=0.5,
    )
    add_colored_aztec_to_canvas(
...
def generate_sd_background(prompt, width=512, height=512, save_path="./backgrounds", seed_text="", denomination=None):
    ...
    save_path = str(save_path).strip().strip('"').strip("'") or "./backgrounds"
    os.makedirs(save_path, exist_ok=True)
    ...
    try:
        ...
        if images:
            ...
            denom_value = str(denomination) if denomination else "node"
            denom_clean = re.sub(r"[^A-Za-z0-9_-]+", "", denom_value)
            denom_str = f"d{denom_clean}" if denom_clean else "node"
...
def generate_sd_background_from_metadata(encoded_seed, name="", width: int = 512, height: int = 512, 
    ...
    try:
        ...
        if images:
            ...
            denom_value = str(denom) if denom else "node"
            denom_clean = re.sub(r"[^A-Za-z0-9_-]+", "", denom_value)
            denom_str = f"d{denom_clean}" if denom_clean else "node"
...
def run_single_denomination(outdir: str = ".", base_name: str = "banknote", denomination: int = 1, 
                           ...
                           progress_callback=None):
    print(f"[DEBUG] run_single_denomination called with outdir={outdir}, base_name={base_name}, denomination={denomination}, width_mm={width_mm}, height_mm={height_mm}, title_text={title_text}, phrase_text={phrase_text}, seed_text={seed_text}, serial_id={serial_id}, timestamp={timestamp}, png={png}, dpi={dpi}, bg_image={bg_image}")
    # Update global DPI
    global MM_TO_PX
    MM_TO_PX = dpi / 25.4
    ...
    print(f"[DEBUG] run_single_denomination: Will write SVG to {path}")
    try:
        ...
        print(f"[ERROR] Exception in generate_backside_svg: {e}")
        import traceback
        traceback.print_exc()
    print(f"[DEBUG] run_single_denomination: Checking if SVG exists at {path}: {os.path.exists(path)}")
    if not os.path.exists(path):
        print(f"[ERROR] SVG was not created at {path}")
    else:
        print(f"[DEBUG] SVG successfully created at {path}")
    if png:
