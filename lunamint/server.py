"""HTTP server for EisenScript rendering."""
...
from lunamint.scripting.eisen import parse_script, render_html, render_svg
...
def serve(host: str, port: int) -> None:
    server = ThreadingHTTPServer((host, port), EisenHandler)
    print(f"[+] EisenScript API listening on http://{host}:{port}")
    try:
...
def main() -> int:
    ...
    parser.add_argument("--port", type=int, default=4242, help="Bind port")
